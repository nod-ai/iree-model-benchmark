...
# Usage: PATH=/path/to/iree/build/tools:$PATH ./benchmark-70b-fp4-decode.sh N [<irpa-path>]
...
if (( "${USE_TRACY}" == "1")); then
    ...
    "${TRACY_CAPTURE}" -f -p ${TRACY_PORT} -o "${WORKING_DIR}/${PREFIX}.decode.70b_fp4.tracy"
else
    run_benchmark
fi
